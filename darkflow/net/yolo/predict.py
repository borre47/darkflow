...
def process_box(self, b, h, w, threshold):
	...
	if max_prob > threshold:
		...
		if bot   > h - 1:   bot = h - 1
		mess = '{}'.format(label)
      #label_probs = { self.meta['labels'][ind] : bi for ind,bi in enumerate(b.probs) }
		return (left, right, top, bot, mess, max_indx, max_prob,{self.meta['labels'][ind] : bi for ind,bi in enumerate(b.probs)})
	return None
